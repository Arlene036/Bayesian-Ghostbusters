...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            ...
            # If the factor has more than one unconditioned variable after joining, eliminate the variable
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        non_negative_values = [max(0, v) for v in values]
        sampled_key = random.choices(keys, weights=non_negative_values, k=1)[0]
        return sampled_key
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # print('self.beliefs', self.beliefs) # updata this!!
        # print('self.allPositions', self.allPositions) # a list of the possible ghost positions
        # print('gameState', gameState)
        # print('self.getObservationProb (example)', self.getObservationProb(observation,gameState.getPacmanPosition(),self.allPositions[0],self.getJailPosition()))
        # print('gameState.getPacmanPosition', gameState.getPacmanPosition())
        # print('self.getJailPosition',self.getJailPosition())
        # print('~~~~~~~~~~~~~~~~~~~~~~')
        pacman_position = gameState.getPacmanPosition()
        for belief in self.beliefs:
            possible_ghost_position = belief # tuple
            prob_e_given_x = self.getObservationProb(observation, pacman_position, possible_ghost_position, self.getJailPosition())
            self.beliefs[belief] = prob_e_given_x * self.beliefs[belief]
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist:
                self.beliefs[newPos] = newPosDist[newPos] * old_beliefs[oldPos] + self.beliefs[newPos]
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(num_particles):
            r = random.randint(0, num_legal_positions - 1)
            self.particles.append(self.legalPositions[r])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            ...
            belief_distribution[particle] += observation_prob

        if belief_distribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [belief_distribution.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
